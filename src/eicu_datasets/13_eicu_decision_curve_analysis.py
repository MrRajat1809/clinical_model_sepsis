...
Phase 12: Decision Curve Analysis (DCA) on External Validation Cohort
Evaluates the true clinical utility of the model by calculating Net Benefit.
1. Recreates the exact 80/20 Calibration/Test split for eICU.
2. Applies Platt Scaling and Isotonic Regression.
3. Computes Net Benefit for Uncalibrated, Platt, Isotonic, Treat All, and Treat None.
4. Generates a TRIPOD-compliant DCA plot, marking the optimal clinical thresholds.
"""
...
OUT_PLOT = BASE_DIR / "results" / "eicu_external_validation" / "eicu_dca_plot.png"
OUT_CSV = BASE_DIR / "results" / "eicu_external_validation" / "eicu_dca_summary.csv"
...
def main():
    ...
    # 1. Load & Split Data (Exact match to script 11)
