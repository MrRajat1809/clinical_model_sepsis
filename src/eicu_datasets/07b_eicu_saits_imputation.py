...
Phase 9: External Validation (Locked SAITS Imputation)
...
- Loads the fitted StandardScaler from the MIMIC-IV metadata directory.
- Loads the locked SAITS model weights from the MIMIC-IV metadata directory.
...
# eICU Paths (Input/Output)
EICU_TENSOR_DIR = BASE_DIR / "data" / "processed" / "eicu" / "tensors"
EICU_META_DIR = EICU_TENSOR_DIR / "metadata"

# MIMIC-IV Paths (Locked Artifacts)
MIMIC_META_DIR = BASE_DIR / "data" / "processed" / "mimiciv" / "tensors" / "metadata"
...
def run_saits_inference():
    ...
    EICU_META_DIR.mkdir(parents=True, exist_ok=True)
    
    # Inputs (eICU Data)
    raw_file = EICU_TENSOR_DIR / "eicu_sepsis_tensor_raw.npy"
    feature_file = EICU_TENSOR_DIR / "eicu_sepsis_tensor_features.npy"
    
    # Locked Artifacts (MIMIC-IV)
    locked_scaler_file = MIMIC_META_DIR / "saits_scaler.joblib"
    locked_weights_file = MIMIC_META_DIR / "saits_model_weights.pypots"
    
    # Outputs (eICU Imputed Data)
    out_file = EICU_TENSOR_DIR / "eicu_sepsis_imputed_tensor.npy"
    mask_file = EICU_META_DIR / "eicu_saits_missingness_mask.npy"
    qc_file = EICU_META_DIR / "eicu_sepsis_tensor_qc_report.csv"
    plot_file = EICU_META_DIR / "eicu_imputation_distributions.png"
    ...
    if not locked_scaler_file.exists() or not locked_weights_file.exists():
        print("[ERROR] Locked MIMIC-IV SAITS artifacts not found. Cannot proceed with external validation.")
        return
