...
Phase 9: External Validation (eICU Shape Trajectory Manifold)
...
TENSOR_DIR = BASE_DIR / "data" / "processed" / "eicu" / "tensors"
COHORT_DIR = BASE_DIR / "data" / "processed" / "eicu"

# Shape Atlas Directories
ATLAS_SHAPE_DIR = BASE_DIR / "data" / "processed" / "eicu" / "atlas" / "shape"
FIGURE_DIR = ATLAS_SHAPE_DIR / "figures"

def run_shape_atlas_characterization():
    print("[*] Initializing eICU PHATE Manifold Projection (Shape/Morphology Space)...")
    start_time = time.time()
    
    FIGURE_DIR.mkdir(parents=True, exist_ok=True)
    
    # Inputs
    dist_matrix_file = BASE_DIR / "data" / "processed" / "eicu" / "atlas" / "eicu_dtw_pairwise_distance_matrix.npy"
    stay_id_file = TENSOR_DIR / "eicu_sepsis_tensor_stay_ids.npy"
    cohort_file = COHORT_DIR / "eicu_final_sepsis3_cohort.parquet"
    tensor_file = TENSOR_DIR / "eicu_sepsis_imputed_tensor.npy"
    raw_tensor_file = TENSOR_DIR / "eicu_sepsis_tensor_raw.npy"  
    features_file = TENSOR_DIR / "eicu_sepsis_tensor_features.npy"
    
    # Outputs
    atlas_coords_file = ATLAS_SHAPE_DIR / "eicu_phate_shape_coordinates.parquet"
    plot_file = FIGURE_DIR / "eICU_Figure_2_Shape_Trajectory_Manifold.png"
    ...
    axes[0].set_title("A. eICU Mortality (Shape Manifold)", fontsize=14, weight='bold')
    ...
    print(f"\n[+] Success! eICU Manifold embedded and multi-panel shape figure generated.")
...
if __name__ == "__main__":
    run_shape_atlas_characterization()