...
Phase 9: External Validation (eICU Trajectory Manifold)
Computes the pairwise Dynamic Time Warping (DTW) distance matrix for the fully imputed 
eICU 3D time-series tensor.

[METHODOLOGY NOTE]: Uses TimeSeriesScalerMeanVariance(). Because this scales each patient's 
...
ATLAS_DIR = BASE_DIR / "data" / "processed" / "eicu" / "atlas"
...
def run_tensor_atlas():
    ...
    # Atlas Outputs
    distance_matrix_file = ATLAS_DIR / "eicu_dtw_pairwise_distance_matrix.npy"
    scaler_file = META_DIR / "eicu_dtw_scaler.joblib"
    ...
    # 4. Serialization
    print("    -> Serializing eICU pairwise distance matrix...")
