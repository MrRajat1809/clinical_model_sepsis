"""
09a_eicu_DTW_atlas_clinical.py

Phase 9: External Validation (eICU Clinical/Severity Manifold)
Computes the pairwise Dynamic Time Warping (DTW) distance matrix for the fully imputed 
eICU 3D time-series tensor based on ABSOLUTE CLINICAL STATE.

[METHODOLOGY NOTE]: Applies a global 2D StandardScaler across all patients and timesteps. 
...
ATLAS_CLINICAL_DIR = BASE_DIR / "data" / "processed" / "eicu" / "atlas" / "severity"
...
def run_clinical_atlas():
    ...
    # Atlas Outputs
    distance_matrix_file = ATLAS_CLINICAL_DIR / "eicu_dtw_pairwise_distance_matrix.npy"
    atlas_ids_file = ATLAS_CLINICAL_DIR / "eicu_atlas_stay_ids.npy"
    scaler_file = META_DIR / "eicu_global_clinical_scaler.joblib"
    metadata_file = META_DIR / "eicu_atlas_metadata.json"
