...
Phase 9: External Validation (eICU Vasopressor Standardization)
...
- Relies on an external JSON configuration file for standard ICU concentration assumptions 
  to salvage volumetric (ml/hr) rates.
- Isolates and exports unprocessable rows for debugging and manual review.
...
AUDIT_DIR = PROCESSED_DIR / "profiling"

CONFIG_DIR.mkdir(parents=True, exist_ok=True)
AUDIT_DIR.mkdir(parents=True, exist_ok=True)

CONFIG_FILE = CONFIG_DIR / "standard_pressor_concentrations.json"
...
def standardize_units():
    ...
    unconvertible_file = AUDIT_DIR / "unprocessable_pressors.csv"
