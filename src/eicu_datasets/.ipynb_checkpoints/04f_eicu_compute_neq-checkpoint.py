...
Phase 9: External Validation (eICU NEQ Computation & Feature Equivalence)
...
This script assumes the MIMIC-IV pipeline (Phase 7) is concurrently updated 
to match this exact formula to prevent covariate shift.
...
- Exports a 'feature_equivalence_report.json'.
"""
...
MIMIC_DIR = BASE_DIR / "data" / "processed" / "mimiciv" / "tensors"
AUDIT_DIR = EICU_DIR / "profiling"
...
def compute_neq_and_validate():
    ...
    in_file = EICU_DIR / "eicu_standardized_pressors.parquet"
    out_file = EICU_DIR / "eicu_neq_timeline.parquet"
    equiv_report_file = AUDIT_DIR / "feature_equivalence_report_NEQ.json"
    equiv_plot_file = AUDIT_DIR / "MIMIC_vs_eICU_NEQ_Distribution.png"
    ...
    mimic_tensor_file = MIMIC_DIR / "sepsis_imputed_tensor.npy"
    mimic_features_file = MIMIC_DIR / "sepsis_tensor_features.npy"
    ...
    if mimic_tensor_file.exists() and mimic_features_file.exists():
        ...
        if "neq" in features:
            ...
            plt.close()
            print(f"       - Distribution plot saved to: {equiv_plot_file.name}")
        else:
