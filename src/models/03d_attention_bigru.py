...
- Output: Exports trained model, predictions, learning curves, and attention-weighted 
  temporal embeddings for downstream hybrid stacking.
"""
...
def main():
    ...
    epochs_no_improve = 0
    history = []
    model_save_path = OUT_MODELS / "attention_multimodal_bigru_best.pth"
    ...
    df_hist.to_csv(OUT_LOGS / "attention_multimodal_bigru_history.csv", index=False)
    ...
    plt.savefig(OUT_LOGS / "attention_multimodal_bigru_curve.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    np.save(OUT_EMBEDDINGS / "attention_multimodal_bigru_embeddings.npy", X_embeddings)
    np.save(OUT_EMBEDDINGS / "attention_multimodal_bigru_stay_ids.npy", stay_ids)
    ...
    df_preds.to_csv(OUT_PREDS / "attention_multimodal_bigru_predictions.csv", index=False)
    ...
    with open(OUT_MODELS / "attention_multimodal_bigru_config.json", "w") as f:
    ...
    with open(OUT_METRICS / "attention_multimodal_bigru_metrics.json", "w") as f:
