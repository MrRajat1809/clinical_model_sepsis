"""
09b_eicu_pruned_external_validation.py

Phase 11: External Validation (Pruned 72-Feature Model - Pure Inference)
Loads the locked, lightweight 72-feature XGBoost model trained on MIMIC-IV and 
evaluates it on the eICU external validation cohort.

Strictly enforces external validation standards:
- Loads pre-trained model weights (NO retraining).
- Dynamically reconstructs the MIMIC-IV StandardScalers to ensure the eICU data 
  is transformed into the exact same statistical distribution space.
...
EICU_TENSOR_FILE = EICU_TENSOR_DIR / "eicu_sepsis_imputed_tensor.npy"
EICU_STAY_ID_FILE = EICU_TENSOR_DIR / "eicu_sepsis_tensor_stay_ids.npy"
...
MIMIC_SPLITS_DIR = BASE_DIR / "outputs" / "baselines" / "train_test_split"

# Feature Cutoff & Model Files
FEAT_NAMES_FILE = BASE_DIR / "outputs" / "champion" / "feature_names" / "champion_features.json"
STABLE_FEATS_FILE = BASE_DIR / "outputs" / "rfecv" / "stable_optimal_features.json"
PRUNED_MODEL_FILE = BASE_DIR / "outputs" / "pruned" / "models" / "pruned_champion_xgboost.joblib"

# Outputs
RESULTS_DIR = BASE_DIR / "outputs" / "eicu_external_validation"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

METRICS_FILE = RESULTS_DIR / "eicu_pruned_metrics.json"
ROC_PLOT_FILE = RESULTS_DIR / "eICU_Figure_5_Pruned_ROC_PR.png"
PREDS_FILE = RESULTS_DIR / "eicu_pruned_predictions.csv"
...
# ==========================================
def main():
    print("[*] Initiating Phase 11: External Validation (Pruned 72-Feature Model - Pure Inference)...")
    ...
    # ---------------------------------------------------------
    # 1. RECONSTRUCT SCALERS & LOAD FEATURE LIST
    # ---------------------------------------------------------
    print("    -> Reconstructing MIMIC-IV scalers and isolating the 72 stable features...")
    ...
    mimic_tensor = np.load(MIMIC_TENSOR_DIR / "sepsis_imputed_tensor.npy")
    mimic_stay_ids = np.load(MIMIC_TENSOR_DIR / "sepsis_tensor_stay_ids.npy")
    mimic_train_idx = np.load(MIMIC_SPLITS_DIR / "train_indices.npy")
    
    df_mimic = pl.read_parquet(MIMIC_COHORT_DIR / "final_sepsis3_cohort.parquet").to_pandas()
    ...
    # ---------------------------------------------------------
    # 2. EXTRACT, SCALE, AND PRUNE eICU DATA
    # ---------------------------------------------------------
    ...
    df_eicu = pl.read_parquet(EICU_COHORT_DIR / "eicu_final_sepsis3_cohort.parquet").to_pandas()
    ...
    # Scale using the locked MIMIC scaler, then filter to 72 features
    ...
    # ---------------------------------------------------------
    # 3. INFERENCE & EVALUATION ON eICU
    # ---------------------------------------------------------
    ...
    with open(METRICS_FILE, "w") as f:
        json.dump({"model": "Pruned_72_Feature_External_Validation", "metrics": metrics}, f, indent=4)
