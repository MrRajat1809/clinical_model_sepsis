"""
09_evaluate_pruned_model.py

Phase 10: Pruned Model Validation & Export
...
# Outputs
PRUNED_OUT_DIR = BASE_DIR / "outputs" / "pruned" / "models"
PRUNED_OUT_DIR.mkdir(parents=True, exist_ok=True)
PRUNED_MODEL_FILE = PRUNED_OUT_DIR / "pruned_champion_xgboost.joblib"
...
def main():
    ...
    # ---------------------------------------------------------
    # 3. FILTER DOWN TO THE 72 PRUNED FEATURES
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    print("    -> Evaluating the original 124-feature Champion model...")
    ...
    print("======================================================================")
    print(f" Original Champion (124 Features) : AUROC = {champion_auroc:.4f} | AUPRC = {champion_auprc:.4f}")
    print(f" Pruned Model       (72 Features) : AUROC = {pruned_auroc:.4f} | AUPRC = {pruned_auprc:.4f}")
    print("======================================================================")
    
    diff = pruned_auroc - champion_auroc
    if diff >= 0:
        print(f" [*] SUCCESS: Pruned model matched/improved AUROC by +{diff:.4f} while shedding 52 features!")
    else:
        print(f" [*] SUCCESS: Pruned model shed 52 features with only a marginal AUROC shift of {diff:.4f}.")
