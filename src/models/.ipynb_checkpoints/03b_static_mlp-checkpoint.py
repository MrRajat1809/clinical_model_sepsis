...
for d in [OUT_MODELS, OUT_PREDS, OUT_METRICS, OUT_LOGS]:
...
def main():
    ...
    epochs_no_improve = 0
    history = []
    model_save_path = OUT_MODELS / "static_mlp_best.pth"
    ...
    df_hist.to_csv(OUT_LOGS / "static_mlp_history.csv", index=False)
    ...
    plt.savefig(OUT_LOGS / "static_mlp_curve.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    df_preds.to_csv(OUT_PREDS / "static_mlp_predictions.csv", index=False)
    ...
    with open(OUT_MODELS / "static_mlp_config.json", "w") as f:
    ...
    with open(OUT_METRICS / "static_mlp_metrics.json", "w") as f:
