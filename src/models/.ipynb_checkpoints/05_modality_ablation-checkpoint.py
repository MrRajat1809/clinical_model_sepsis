...
# Inputs
CHAMPION_METRICS_FILE = BASE_DIR / "outputs" / "champion" / "metrics" / "champion_metrics.json"
LATENT_EMBEDDINGS_FILE = BASE_DIR / "outputs" / "deep_learning" / "embeddings" / "temporal_bigru_embeddings.npy"

# Outputs
OUT_MODELS = BASE_DIR / "outputs" / "ablation" / "models"
OUT_PREDS = BASE_DIR / "outputs" / "ablation" / "predictions"
OUT_METRICS = BASE_DIR / "outputs" / "ablation" / "metrics"
OUT_PLOTS = BASE_DIR / "outputs" / "ablation" / "plots"

for d in [OUT_MODELS, OUT_PREDS, OUT_METRICS, OUT_PLOTS]:
...
def main():
    ...
    for category, models in combinations.items():
        for name, X_combo in models.items():
            ...
            joblib.dump(model, OUT_MODELS / f"ablation_{safe_name}.joblib")
            
            pd.DataFrame({
                "stay_id": stay_ids_test,
                "true_label": y_test,
                "pred_probability": preds
            }).to_csv(OUT_PREDS / f"{safe_name}.csv", index=False)
    ...
    df_results.to_csv(OUT_METRICS / "modality_metrics.csv", index=False)
    
    with open(OUT_METRICS / "ablation_metadata.json", "w") as f:
    ...
    plt.tight_layout()
    plt.savefig(OUT_PLOTS / "ablation_plot.png", dpi=300, bbox_inches='tight')
    plt.close()
