"""
03b_multimodal_bigru.py

Phase 2: Test Whether Deep Learning Actually Helps
Trains a Multimodal Deep Learning Model (BiGRU + Static MLP).
- Input: 24h physiological time series tensor + Static clinical context.
- Purpose: Benchmarks whether a hybrid neural network can outperform 
  the classical ML Super-Stack baseline when given identical information.
- Output: Exports trained model, predictions, learning curves, and temporal 
  embeddings for downstream hybrid stacking.
"""
...
def main():
    ...
    epochs_no_improve = 0
    history = []
    model_save_path = OUT_MODELS / "multimodal_bigru_best.pth"
    ...
    df_hist.to_csv(OUT_LOGS / "multimodal_bigru_history.csv", index=False)
    ...
    plt.savefig(OUT_LOGS / "multimodal_bigru_curve.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    np.save(OUT_EMBEDDINGS / "multimodal_bigru_embeddings.npy", X_embeddings)
    np.save(OUT_EMBEDDINGS / "multimodal_bigru_stay_ids.npy", stay_ids)
    ...
    df_preds.to_csv(OUT_PREDS / "multimodal_bigru_predictions.csv", index=False)
    ...
    with open(OUT_MODELS / "multimodal_bigru_config.json", "w") as f:
    ...
    with open(OUT_METRICS / "multimodal_bigru_metrics.json", "w") as f:
