...
# Outputs
OUT_MODELS = BASE_DIR / "outputs" / "calibration" / "models"
OUT_PREDS = BASE_DIR / "outputs" / "calibration" / "predictions"
OUT_METRICS = BASE_DIR / "outputs" / "calibration" / "metrics"
OUT_PLOTS = BASE_DIR / "outputs" / "calibration" / "plots"
OUT_PLOT_DATA = BASE_DIR / "outputs" / "calibration" / "plot_data"

for d in [OUT_MODELS, OUT_PREDS, OUT_METRICS, OUT_PLOTS, OUT_PLOT_DATA]:
...
def main():
    ...
    # Export Calibrators
    joblib.dump(platt_calibrator, OUT_MODELS / "platt_calibrator.joblib")
    joblib.dump(iso_calibrator, OUT_MODELS / "isotonic_calibrator.joblib")
    ...
    df_preds.to_csv(OUT_PREDS / "06_calibrated_predictions.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(OUT_PLOTS / "calibration_curves.png", dpi=300, bbox_inches='tight')
    plt.close()

    # Save curve data for external plotting
    pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in plot_data_curves.items() ])).to_csv(OUT_PLOT_DATA / "calibration_curve_data.csv", index=False)

    # ---------------------------------------------------------
    # 9. JSON EXPORT & PRINT RESULTS
    # ---------------------------------------------------------
    with open(OUT_METRICS / "calibration_summary.json", "w") as f:
