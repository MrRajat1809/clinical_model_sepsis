...
- Exports exact train/test indices and stay_ids.
...
# Structured outputs based on the new paradigm
OUT_MODELS = BASE_DIR / "outputs" / "baselines" / "models"
OUT_PREDS = BASE_DIR / "outputs" / "baselines" / "predictions"
OUT_METRICS = BASE_DIR / "outputs" / "baselines" / "metrics"
OUT_FEATS = BASE_DIR / "outputs" / "baselines" / "feature_names"
OUT_SPLITS = BASE_DIR / "outputs" / "baselines" / "train_test_split"

for d in [OUT_MODELS, OUT_PREDS, OUT_METRICS, OUT_FEATS, OUT_SPLITS]:
...
def main():
    ...
    # Save Feature Names
    with open(OUT_FEATS / "static_features.json", "w") as f: json.dump(static_cols, f)
    with open(OUT_FEATS / "aggregated_features.json", "w") as f: json.dump(agg_names, f)
    with open(OUT_FEATS / "combined_features.json", "w") as f: json.dump(combined_names, f)
    ...
    # Save Splits
    np.save(OUT_SPLITS / "train_indices.npy", idx_train)
    np.save(OUT_SPLITS / "test_indices.npy", idx_test)
    np.save(OUT_SPLITS / "stay_ids_train.npy", stay_ids[idx_train])
    np.save(OUT_SPLITS / "stay_ids_test.npy", stay_ids[idx_test])
    ...
    for name, (model, X_tr, X_te) in models_config.items():
        ...
        pd.DataFrame({"stay_id": stay_ids_test, "true_label": y_test, "pred_probability": preds, "pred_label": (preds >= 0.5).astype(int)}).to_csv(OUT_PREDS / f"{name}.csv", index=False)
    ...
    joblib.dump(xgb_model, OUT_MODELS / "XGBoost_Combined.joblib")
    pd.DataFrame({"stay_id": stay_ids_test, "true_label": y_test, "pred_probability": preds_xgb, "pred_label": (preds_xgb >= 0.5).astype(int)}).to_csv(OUT_PREDS / "XGBoost_Combined.csv", index=False)
    ...
    # ---------------------------------------------------------
    with open(OUT_METRICS / "detailed_baseline_metrics.json", "w") as f:
        json.dump(full_metrics_dict, f, indent=4)
        
    df_results = pd.DataFrame(results_list).sort_values(by="AUROC", ascending=False)
    df_results.to_csv(OUT_METRICS / "baseline_results_summary.csv", index=False)
    ...
    print(f"    -> Exported models, predictions, feature names, and split indices to outputs/baselines/")
