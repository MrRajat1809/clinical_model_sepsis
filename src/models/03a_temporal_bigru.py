...
- Output: Exports trained model, predictions, learning curves, and full dataset 
  latent embeddings for downstream hybrid stacking.
"""
...
OUT_MODELS = BASE_DIR / "outputs" / "deep_learning" / "models"
OUT_PREDS = BASE_DIR / "outputs" / "deep_learning" / "predictions"
OUT_METRICS = BASE_DIR / "outputs" / "deep_learning" / "metrics"
OUT_EMBEDDINGS = BASE_DIR / "outputs" / "deep_learning" / "embeddings"
OUT_LOGS = BASE_DIR / "outputs" / "deep_learning" / "training_logs"

for d in [OUT_MODELS, OUT_PREDS, OUT_METRICS, OUT_EMBEDDINGS, OUT_LOGS]:
...
def set_seed(seed):
    """Enforces strict determinism for reproducible deep learning."""
    random.seed(seed)
    np.random.seed(seed)
...
class TemporalSepsisDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.X_temp[idx], self.y[idx]

class TemporalBiGRUNet(nn.Module):
    def __init__(self, temporal_dim=30, hidden_dim=64):
        super(TemporalBiGRUNet, self).__init__()
    ...
    def forward(self, x_temp):
        ...
        # Average pooling across 24h trajectory
...
def main():
    ...
    stay_ids_test = np.load(SPLITS_DIR / "stay_ids_test.npy")

    # Split train_val into train and val (85% train, 15% val of the train_val subset)
    y_train_val = y[idx_train_val]
    idx_train, idx_val = train_test_split(
        idx_train_val, test_size=0.15, random_state=RANDOM_STATE, stratify=y_train_val 
    )

    X_train, y_train = X_imputed[idx_train], y[idx_train]
    X_val, y_val = X_imputed[idx_val], y[idx_val]
    X_test, y_test = X_imputed[idx_test], y[idx_test]

    train_loader = DataLoader(TemporalSepsisDataset(X_train, y_train), batch_size=BATCH_SIZE, shuffle=True)
    val_loader = DataLoader(TemporalSepsisDataset(X_val, y_val), batch_size=BATCH_SIZE)
    ...
    epochs_no_improve = 0
    history = []
    model_save_path = OUT_MODELS / "temporal_bigru_best.pth"
    ...
    df_hist.to_csv(OUT_LOGS / "temporal_bigru_history.csv", index=False)
    ...
    plt.title("BiGRU Training Curve")
    plt.xlabel("Epoch")
    plt.ylabel("Metric")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(OUT_LOGS / "temporal_bigru_curve.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    with torch.no_grad():
        for x_batch, _ in full_loader:
            _, emb = model(x_batch.to(DEVICE))
            all_embeddings.append(emb.cpu().numpy())
    
    X_embeddings = np.concatenate(all_embeddings, axis=0)
    np.save(OUT_EMBEDDINGS / "temporal_bigru_embeddings.npy", X_embeddings)
    np.save(OUT_EMBEDDINGS / "temporal_bigru_stay_ids.npy", stay_ids)
    print(f"       - Saved {X_embeddings.shape[1]}D embeddings for {len(stay_ids)} patients.")
    ...
    df_preds.to_csv(OUT_PREDS / "temporal_bigru_predictions.csv", index=False)
    
    # Export Config
    config = {
        "model": "Temporal_BiGRU",
        "hyperparameters": {"hidden_dim": HIDDEN_DIM, "layers": 2, "dropout": 0.2, "batch_size": BATCH_SIZE, "learning_rate": LEARNING_RATE, "patience": PATIENCE},
        "best_epoch": int(df_hist.loc[df_hist['Val_AUPRC'].idxmax()]['Epoch'])
    }
    with open(OUT_MODELS / "temporal_bigru_config.json", "w") as f:
    ...
    with open(OUT_METRICS / "temporal_bigru_metrics.json", "w") as f:
        json.dump(metrics, f, indent=4)
        
    print("\n" + "="*50)
    print(" UNIMODAL BiGRU PERFORMANCE (TEST SET)")
    print("="*50)
