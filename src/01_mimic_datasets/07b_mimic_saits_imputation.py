...
def run_saits_model():
    ...
    for d in [PROCESSED_DIR, OUT_MODELS_DIR, OUT_METRICS_DIR, OUT_FIGURES_DIR]:
        d.mkdir(parents=True, exist_ok=True)
    
    # Input files from 07a
    raw_file = PROCESSED_DIR / "mimic_sepsis_tensor_raw.npy"
    ...
    # Export paths mapped to strict directory structure
    out_file = PROCESSED_DIR / "mimic_sepsis_imputed_tensor.npy"
    mask_file = PROCESSED_DIR / "mimic_saits_missingness_mask.npy"
    
    scaler_file = OUT_MODELS_DIR / "mimic_saits_scaler.joblib"
    weights_file = OUT_MODELS_DIR / "mimic_saits_model_weights.pypots"
    train_idx_file = OUT_MODELS_DIR / "mimic_saits_train_indices.npy"
    val_idx_file = OUT_MODELS_DIR / "mimic_saits_val_indices.npy"
    config_file = OUT_MODELS_DIR / "mimic_preprocessing_config.json"
    
    qc_file = OUT_METRICS_DIR / "mimic_sepsis_tensor_qc_report.csv"
    plot_file = OUT_FIGURES_DIR / "mimic_imputation_distributions.png"
    ...
    saits = SAITS(
        ...
        saving_path=str(OUT_MODELS_DIR), model_saving_strategy="best"
    )
