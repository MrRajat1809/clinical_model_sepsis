"""
Overlay clinical variables on the shared prognostic manifold.

Turns the joint embedding into an interpretable figure by colouring the same
coordinates by cohort origin, mortality, and a set of physiological variables
chosen to span different organ systems: mean lactate, maximum blood urea
nitrogen, mean aPTT, minimum motor GCS and variability in verbal GCS.

The cohort-origin panel and the mortality panel are the pair worth reading
together: origin should look mixed while mortality should show a gradient. Mixed
origin with a preserved mortality gradient is the outcome that would justify
treating the two cohorts as one population.

Columns are addressed by position within the 122-D layout, where the mean block
occupies the first 30 columns, then minimum, maximum and standard deviation.

Reads:
    the atlas features and metadata, atlas_phate_coordinates.parquet
Writes:
    outputs/figures/atlas prognostic landscape panels
"""
...
def main():
    ...
    assert len(df_coords) == len(df_meta) == len(X_atlas), "CRITICAL: Row dimensions mismatch across Atlas artifacts!"
    ...
    # In 122D: [0-29: Mean], [30-59: Min], [60-89: Max], [90-119: Std], [120-121: Static]
    def extract_val(base_idx, offset):
        if base_idx is not None:
            return X_atlas[:, base_idx + offset]
        return np.zeros(len(X_atlas))
