"""
09b_mimic_atlas_clinical_characterization.py
...
[FIX APPLIED]: Extracted ventilation status from the RAW tensor to bypass continuous neural 
network hallucinations.
[FIX APPLIED]: Shuffled plot order and applied equal-weight muted colors (Green/Purple) 
to Panel G to eliminate the "orange carpet" overplotting effect.
"""
...
# Clinical Atlas Directories
ATLAS_CLINICAL_DIR = BASE_DIR / "data" / "processed" / "mimiciv" / "atlas" / "severity"
FIGURE_DIR = ATLAS_CLINICAL_DIR / "figures"

def run_atlas_characterization():
    print("[*] Initializing PHATE Manifold Projection (Clinical Severity Space)...")
    start_time = time.time()
    
    FIGURE_DIR.mkdir(parents=True, exist_ok=True)
    
    # Inputs
    dist_matrix_file = ATLAS_CLINICAL_DIR / "dtw_pairwise_distance_matrix.npy"
    atlas_ids_file = ATLAS_CLINICAL_DIR / "atlas_stay_ids.npy"
    cohort_file = COHORT_DIR / "final_sepsis3_cohort.parquet"
    tensor_file = TENSOR_DIR / "sepsis_imputed_tensor.npy"
    raw_tensor_file = TENSOR_DIR / "sepsis_tensor_raw.npy"  
    features_file = TENSOR_DIR / "sepsis_tensor_features.npy"
    
    # Outputs
    atlas_coords_file = ATLAS_CLINICAL_DIR / "phate_clinical_coordinates.parquet"
    plot_file = FIGURE_DIR / "Figure_2_Clinical_Trajectory_Manifold.png"
    ...
    print("    -> Loading DTW Distance Matrix and Metadata...")
    ...
    print("    -> Serializing Atlas coordinates...")
    ...
    axes[0].set_title("A. Hospital Mortality", fontsize=14, weight='bold')
    ...
    print(f"\n[+] Success! Manifold embedded and multi-panel clinical figure generated.")
