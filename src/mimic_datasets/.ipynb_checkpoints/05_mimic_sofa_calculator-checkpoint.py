...
[FIX APPLIED]: Removed synthetic NEQ approximation. Implemented exact, strict Sepsis-3 
...
def main():
    ...
    out_file = PROCESSED_DIR / "final_sepsis3_cohort.parquet"
    
    if not temporal_file.exists():
        print(f"[ERROR] Cleaned temporal data not found at: {temporal_file}")
        return

    print("\n[*] Initializing Polars lazy engine and mapping variables...")
    ...
    }, schema={"itemid": pl.Int64, "variable": pl.Utf8}).lazy()
    
    # Filter vitals to just SOFA components and map names
    df_mapped = df_vitals.join(mapping_df, on="itemid", how="inner")
    
    # Join with cohort to get SIT
    df_joined = df_mapped.join(df_cohort.lazy().select(["stay_id", "suspected_infection_time"]), on="stay_id")

    print("[*] Segmenting temporal data into Baseline (-48h to SIT) and Acute (SIT to +24h) windows...")
    
    # Define dynamic windows relative to SIT
    df_windows = df_joined.with_columns(
        ((pl.col("event_time") - pl.col("suspected_infection_time")).dt.total_hours()).alias("hours_from_sit")
    ).filter(
    ...
    df_agg = df_windows.group_by(["stay_id", "window", "variable"]).agg(
        ...
        pl.when(pl.col("variable").is_in(["map", "gcs_eye", "gcs_verbal", "gcs_motor", "platelets", "pao2"]))
          .then(pl.col("min_val"))
    ...
    df_sofa = df_sofa.with_columns(
        ...
        # Strict Sepsis-3 Cardiovascular SOFA thresholds
        pl.when(
    ...
    # Set final Sepsis Onset Time to max(SIT, deterioration_time)
    final_cohort = final_cohort.with_columns(
        pl.max_horizontal(
            pl.col("suspected_infection_time"), 
            pl.col("sofa_deterioration_time").fill_null(pl.col("suspected_infection_time"))
        ).alias("sepsis_onset_time")
    )
    
    final_cohort.write_parquet(out_file)

    elapsed = time.time() - start_time
    print(f"\n[+] Success! Sepsis-3 cohort locked in {elapsed:.2f} seconds.")
