...
- Directed paths to 'atlas/shape' to maintain separation from the clinical atlas.
"""
...
ATLAS_SHAPE_DIR = BASE_DIR / "data" / "processed" / "mimiciv" / "atlas" / "shape"
...
def run_shape_atlas_characterization():
    ...
    # Inputs
    dist_matrix_file = ATLAS_SHAPE_DIR / "dtw_pairwise_distance_matrix.npy"
    stay_id_file = TENSOR_DIR / "sepsis_tensor_stay_ids.npy"
    ...
    # Outputs
    atlas_coords_file = ATLAS_SHAPE_DIR / "phate_shape_coordinates.parquet"
    plot_file = FIGURE_DIR / "Figure_2_Shape_Trajectory_Manifold.png"
    
    if not dist_matrix_file.exists():
        print(f"[ERROR] Distance matrix not found at {dist_matrix_file}")
        print("Please ensure your shape-based DTW matrix is moved to the 'atlas/shape' directory.")
        return
    ...
    axes[0].set_title("A. Hospital Mortality (Shape Manifold)", fontsize=14, weight='bold')
    ...
    print(f"\n[+] Success! Manifold embedded and multi-panel shape figure generated.")
