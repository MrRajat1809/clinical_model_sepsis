...
[FIX APPLIED]: Added extraction of mortality labels (hospital_expire_flag, dod).
[FIX APPLIED]: Added extraction of ICU context (first_careunit).
[FIX APPLIED]: Renamed 'admission_age' to 'age' for downstream compatibility.
"""
...
def main():
    ...
    WITH ranked_icu AS (
        SELECT 
            ...
            outtime, 
            los,
            first_careunit, -- [ADDED] ICU type context
    ...
    base_cohort AS (
        SELECT 
            ...
            r.stay_id,
            p.gender,
            -- MIMIC-IV exact age calculation at admission [RENAMED to 'age']
            ...
            a.hospital_expire_flag, -- [ADDED] Mortality label
            p.dod                   -- [ADDED] Date of death
        FROM ranked_icu r
        ...
        WHERE r.icu_seq = 1 
          AND r.los >= 1.0
          AND a.admission_type != 'ELECTIVE' -- Exclusion for scheduled surgical prophylaxis
    )
