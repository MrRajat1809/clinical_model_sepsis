...
[METHODOLOGY NOTE]: This script applies a global 2D StandardScaler across all patients 
and timesteps. This ensures all physiological variables are weighted equally by the DTW 
distance metric, while preserving the absolute magnitude (severity) of the patient's condition.
[FIX APPLIED]: Saves local copies of stay_ids and JSON metadata for complete atlas self-documentation.
[ARCHITECTURE SHIFT]: Casts outputs to 'clinical/' sub-directory to emphasize multi-organ mapping.
"""
...
def run_clinical_atlas():
    ...
    # Atlas Outputs
    distance_matrix_file = ATLAS_CLINICAL_DIR / "dtw_pairwise_distance_matrix.npy"
    atlas_ids_file = ATLAS_CLINICAL_DIR / "atlas_stay_ids.npy"
    scaler_file = META_DIR / "global_clinical_scaler.joblib"
    metadata_file = META_DIR / "atlas_metadata.json"
