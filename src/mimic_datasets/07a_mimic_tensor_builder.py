...
[FIX APPLIED]: Added Urine Output (UO), aggregated via sum() per 1-hour bin.
[FIX APPLIED]: Replaced hallucinated NEQ logic with the validated Brown et al. 
pharmacological conversion formula, computed dynamically from raw pressors.
[SCIENTIFIC CORRECTION]: Phenylephrine divisor explicitly updated to 2.2 to exactly
match Brown et al. (2013) equivalencies (1 mcg NEQ = 2.2 mcg Phenylephrine).
[FIX APPLIED]: Maintained tensor compactness by dropping raw pressors post-NEQ calculation.
"""
...
def main():
    ...
    # 3D Tensor Outputs
    out_tensor = TENSOR_DIR / "sepsis_tensor_raw.npy"
    ...
    mask_file = TENSOR_DIR / "sepsis_tensor_mask.npy"
    
    # Static & Label Outputs
    static_file = TENSOR_DIR / "sepsis_tensor_static.npy"
    static_feature_file = TENSOR_DIR / "sepsis_tensor_static_features.npy"
