...
[FIX APPLIED]: Replaced outdated `admission_age` with `age`.
[FIX APPLIED]: Removed redundant DuckDB mortality join, utilizing the `hospital_expire_flag` 
already propagated into the final Sepsis-3 cohort file.
"""
...
def main():
    ...
    print("\n[*] Loading Sepsis-3 cohort and scanning temporal data...")
    ...
    print("[*] Segmenting acute window (0 to 24 hours post-onset)...")
    # Filter strictly to the acute window
    ...
    print("\n" + "="*105)
    print("SEYMOUR ENDOTYPE SANITY CHECK RESULTS (k=4)")
    print("="*105)
    print(summary.to_string(index=False))
    print("="*105)
    
    elapsed = time.time() - start_time
    print(f"\n[+] Seymour Endotype verification completed in {elapsed:.2f} seconds.")
